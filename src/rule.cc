...
void Rule::ParseInputs(const std::string_view& inputs_str) {
  ...
  for (auto const& input : WordScanner(inputs_str)) {
    ...
    if (!g_flags.generate_ninja) {
      ...
      input_files = Glob(std::string(input).c_str());
    }
    if (input_files.size() == 0) {
      input_files.push_back(std::string(input));
    }
...
void Rule::ParsePrerequisites(const std::string_view& line,
  ...
  // unless null.
  prereq_string = std::string(line.data(), line.size());
  ...
  std::string target_prereq = prereq_string.substr(0, separator_pos);
...
std::shared_ptr<Rule> Rule::ApplyPattern(Pattern pat, Symbol output) const {
  std::shared_ptr<Rule> new_rule = std::make_shared<Rule>(*this);
  std::string buf;
